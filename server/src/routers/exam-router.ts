...
router.post('/valid-token', exam.validToken)
router.get('/examinees', exam.examinees)
router.post('/valid-exam/:examId(\\d+)', exam.validExam)
router.get('/:examId(\\d+)', exam.checkTokenForExam, exam.checkTokenIfExpired, exam.getExam)
router.post('/event/switch', exam.checkTokenIfExpired, exam.switchEvent)
